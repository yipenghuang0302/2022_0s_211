...
def generate_test_suite():
    ...
    generate_test ( 2, 8, 12, path="./"  )
    generate_test ( 3, 16, 24, path="./"  )
    generate_test ( 4, 32, 48, path="./"  )
    generate_test ( 5, 64, 96, path="./"  )
                    ...
                    if test_findCycle(4,path,verbose):
                        ...
                        if test_findCycle(5,path,verbose):
                            ...
                            for filenum in range(6,12):
                                generate_test ( filenum, 64, 96, path )
                                allpass &= test_findCycle(filenum,path,verbose)
                            if allpass:
                                score += 7
